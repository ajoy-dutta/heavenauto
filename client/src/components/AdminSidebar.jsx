...
export default function AdminSidebar() {
  ...
  // Auto-expand parent dropdown if a sub-item route is active
  useEffect(() => {
    ...
    <>
      ...
      `}>
        ...
        <div className="p-4 border-b border-slate-800/60 flex justify-between items-center bg-slate-950 shadow-sm z-10 relative">
          ...
          >
            <div className="p-1 bg-slate-800 rounded-lg group-hover:scale-105 transition-transform duration-300">
              <img 
                ...
                className="h-9 w-auto rounded object-contain flex-shrink-0 brightness-95 contrast-105" 
              />
            </div>
          ...
          <button 
            onClick={() => setIsMobileMenuOpen(false)}
            className="md:hidden text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 p-1.5 rounded-full transition-colors flex-shrink-0"
          >
            <FiX size={20} />
          </button>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto custom-scrollbar">
          {navItems.map((item) => {
            ...
            if (item.type === "heading") {
              return (
                <div key={item.id} className="text-[11px] text-slate-500 uppercase tracking-widest font-black mb-2 mt-6 px-3">
                  {item.label}
                </div>
              );
            }
            ...
            if (item.type === "link") {
              ...
              return (
                <Link 
                  ...
                  to={item.to} 
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={`group flex items-center gap-3 p-3 rounded-xl border border-transparent transition-all duration-300 text-sm font-bold 
                    ${isActive 
              ...
              return (
                <div key={item.id} className="mb-1">
                  ...
                  <button 
                    ...
                    className={`group w-full flex items-center justify-between p-3 rounded-xl transition-all duration-300 text-sm font-bold border hover:bg-slate-800/60 hover:text-white
                      ${isOpen || hasActiveChild ? 'bg-transparent border-slate-800/40 text-slate-100' : 'bg-transparent border-transparent text-slate-400'}
                    `}
                  >
                  ...
                  {/* Dropdown Sub-Items with slide animation */}
                      ...
                      {item.subItems.map((sub, idx) => {
                        ...
                        return (
                          <Link 
                            ...
                            to={sub.to} 
                            onClick={() => setIsMobileMenuOpen(false)}
                            className={`group flex items-center gap-2.5 p-2.5 rounded-lg transition-all duration-300 text-sm font-medium hover:translate-x-1
                              ${isSubActive 
                                ? 'bg-slate-800 text-white shadow-sm' 
        ...
        <div className="border-t border-slate-800/60 mt-auto bg-slate-950 p-4 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.2)]">
          <button 
            ...
            className="group flex items-center justify-center gap-2 w-full p-3 bg-red-950/30 border border-red-900/30 text-red-400 rounded-xl font-bold transition-all duration-300 hover:bg-red-600 hover:text-white shadow-sm"
          >
